...
[ApiController]
[Route("api/skuvaultaccounts")]
[ApiVersion("1.0")]
public class SkuVaultAccountsController: ControllerBase
{
    private readonly IMediator _mediator;

    public SkuVaultAccountsController(IMediator mediator)
    {
        _mediator = mediator;
    }
    ...
    [ProducesResponseType(typeof(SkuVaultAccountDto), 201)]
    [ProducesResponseType(400)]
    [ProducesResponseType(500)]
    [Consumes("application/json")]
    [Produces("application/json")]
    [HttpPost(Name = "AddSkuVaultAccount")]
    public async Task<ActionResult<SkuVaultAccountDto>> AddSkuVaultAccount([FromBody]SkuVaultAccountForCreationDto skuVaultAccountForCreation)
    {
        var command = new AddSkuVaultAccount.AddSkuVaultAccountCommand(skuVaultAccountForCreation);
        var commandResponse = await _mediator.Send(command);

        return CreatedAtRoute("GetSkuVaultAccount",
    ...
    /// <response code="400">SkuVaultAccount has missing/invalid values.</response>
    /// <response code="500">There was an error on the server while creating the SkuVaultAccount.</response>
    [ProducesResponseType(typeof(SkuVaultAccountDto), 200)]
    [ProducesResponseType(400)]
    [ProducesResponseType(500)]
    [Produces("application/json")]
    ...
    /// </remarks>
    [ProducesResponseType(typeof(IEnumerable<SkuVaultAccountDto>), 200)]
    [ProducesResponseType(400)]
    [ProducesResponseType(500)]
    [Produces("application/json")]
    [HttpGet(Name = "GetSkuVaultAccounts")]
    public async Task<IActionResult> GetSkuVaultAccounts([FromQuery] SkuVaultAccountParametersDto skuVaultAccountParametersDto)
    {
        var query = new GetSkuVaultAccountList.SkuVaultAccountListQuery(skuVaultAccountParametersDto);
        var queryResponse = await _mediator.Send(query);
